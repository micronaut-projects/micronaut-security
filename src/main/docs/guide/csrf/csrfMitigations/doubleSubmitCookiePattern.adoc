...
You can disable the CSRF Cookie repository, which reads the CSRF token from the cookie, by setting `micronaut.security.csrf.repositories.cookie.enabled` to false.
