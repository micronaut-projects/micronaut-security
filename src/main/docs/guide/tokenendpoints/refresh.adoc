...
WARNING: `RefreshTokenPersistence::persistToken` is an `@EventListener` which is invoked synchronously, on the thread that completed authentication, while the login response is being built. With a reactive authentication provider (or an OAuth 2.0 / OpenID Connect flow) that thread is typically a Netty event loop thread. If your implementation performs blocking I/O, such as a database write, you must offload it. Annotating the method with `@ExecuteOn` has no effect on event listeners; use `@Async(TaskExecutors.BLOCKING)` instead.

[source,java]
.Blocking refresh token persistence offloaded to the blocking executor
----
...
@Singleton
class JdbcRefreshTokenPersistence implements RefreshTokenPersistence {

    private final RefreshTokenRepository refreshTokenRepository; // a blocking repository

    JdbcRefreshTokenPersistence(RefreshTokenRepository refreshTokenRepository) {
        this.refreshTokenRepository = refreshTokenRepository;
    }

    @Async(TaskExecutors.BLOCKING) // <1>
    @Override
    public void persistToken(RefreshTokenGeneratedEvent event) {
        refreshTokenRepository.save(event.getAuthentication().getName(), event.getRefreshToken()); // <2>
    }
...
----
<1> `@Async` executes the method on the named executor and returns immediately, so the event loop is not blocked.
<2> Blocking I/O is now safe.

NOTE: Because the persistence is asynchronous, the login response may be sent before the refresh token has been persisted. Exceptions thrown by an `@Async` method are logged and do not fail the login.
...
include::{includedir}configurationProperties/io.micronaut.security.endpoints.OauthControllerConfigurationProperties.adoc[]
