...
IMPORTANT: `OpenIdClientFactory` will block the event loop while it fetches the metadata from the configured OpenId providers configuration endpoints, which can cause runtime issues in certain cases. If you want to avoid these, you can set `micronaut.security.oauth2.openid.eager-provider-init` to `true`, which will force these calls to happen before the application starts.
