...
|api:security.oauth2.endpoint.authorization.state.persistence.StatePersistence[] |Stores the state to be retrieved later to allow validation |api:security.oauth2.endpoint.authorization.state.persistence.cookie.CookieNoncePersistence[]
|===
