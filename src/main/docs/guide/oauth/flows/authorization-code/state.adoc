...
NOTE: The `SameSite` attribute of the state cookie is not set by default. You can set it with `micronaut.security.oauth2.state.cookie.cookie-same-site` (`Strict`, `Lax` or `None`). If you use `response_mode=form_post`, the authorization server posts the callback cross-site, so you may need `cookie-same-site: None`. Browsers only accept `SameSite=None` cookies that are also `Secure`, so set `micronaut.security.oauth2.state.cookie.cookie-secure: true` too. The PKCE (`micronaut.security.oauth2.pkce.cookie.*`) and nonce (`micronaut.security.oauth2.openid.nonce.cookie.*`) cookies support the same property.
