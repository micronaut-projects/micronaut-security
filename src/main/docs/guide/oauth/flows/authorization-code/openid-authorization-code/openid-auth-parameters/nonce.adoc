...
NOTE: Cookie persistence supports only one in-flight login per browser, whereas session persistence keeps the nonces of several in-flight logins, keyed by the state of the login flow they belong to. See the <<state, state>> documentation for details.
