...
NOTE: The `SameSite` attribute of the nonce cookie is not set by default. You can set it with `micronaut.security.oauth2.openid.nonce.cookie.cookie-same-site` (`Strict`, `Lax` or `None`). `SameSite=None` requires `Secure`, so set `micronaut.security.oauth2.openid.nonce.cookie.cookie-secure: true` too.
