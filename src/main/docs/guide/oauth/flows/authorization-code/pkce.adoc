...
The code verifier is stored until the callback with a api:security.oauth2.endpoint.authorization.pkce.persistence.PkcePersistence[] bean. By default, it is stored in a HTTP cookie, which supports only one in-flight login per browser. Set `micronaut.security.oauth2.pkce.persistence` to `session` to store it in the HTTP session instead, which keeps the code verifiers of several in-flight logins, keyed by the state of the login flow they belong to. See the <<state, state>> documentation for details.
