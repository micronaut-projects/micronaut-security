...
IMPORTANT: The absolute callback URL (the `redirect_uri` sent to the provider and repeated in the authorization code exchange) and the OpenID Connect `post_logout_redirect_uri` are built with api:http.server.util.HttpHostResolver[]. By default it derives the scheme, host and port from the request `Host` header and the `Forwarded` / `X-Forwarded-*` headers, so a client can influence them. When the application runs behind a reverse proxy, pin the host resolution with the `micronaut.server.host-resolution.*` properties (for example `host-allow-list`, `host-header`, `protocol-header` and `port-header`), or set `micronaut.security.oauth2.base-url` (for example `https://app.example.com`) so those URLs use a fixed base regardless of the request headers. A `callback-uri` that is already an absolute URL is used as is.
