...
snippet::io.micronaut.security.docs.rejection.MyRejectionHandler[tags="clazz"]

[.lang-python]
--
A Python class cannot extend a Java class such as `DefaultAuthorizationExceptionHandler`, so the Python replacement implements the `ExceptionHandler` interface and delegates to an instance of the default handler.
--
