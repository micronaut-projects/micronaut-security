...
== Micronaut Security 3.2 breaking changes

`ExceptionHandler` responses are now passed through the current `ErrorResponseProcessor`.
Classes that extend the affected APIs should switch to use the new constructor.
