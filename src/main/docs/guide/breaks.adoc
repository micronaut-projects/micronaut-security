...
== Micronaut Security 5.4 breaking changes

=== Token cookies default to `SameSite=Lax`

The JWT access token cookie (`micronaut.security.token.cookie`) and the refresh token cookie (`micronaut.security.token.refresh.cookie`) are now issued with the `SameSite=Lax` attribute by default. Previously, no `SameSite` attribute was set.

To keep the previous behaviour or use a different value, set `cookie-same-site` explicitly. Allowed values are `Strict`, `Lax` or `None`:
...
NOTE: `SameSite=None` requires the cookie to also be `Secure`.

The `SameSite` attribute cannot be unset via configuration because the property is bound to the `io.micronaut.http.cookie.SameSite` enum. To omit the attribute, invoke `setCookieSameSite(null)` on the configuration bean, for example via a `BeanCreatedEventListener<TokenCookieConfigurationProperties>`.

=== Warning when token cookies are issued without `Secure`

`cookie-secure` still defaults to the secure status of the request, so plain HTTP local development keeps working. However, if a login issues a token cookie without the `Secure` attribute over a request that was not detected as secure, api:security.token.cookie.CookieLoginHandler[] now logs a `WARN` once. Behind a TLS-terminating proxy, set `micronaut.security.token.cookie.cookie-secure=true` and `micronaut.security.token.refresh.cookie.cookie-secure=true`, or configure forwarded-header handling so that requests are detected as secure.
...
- The Singleton constructor `io.micronaut.security.token.jwt.validator.JwtTokenValidator(Collection<SignatureConfiguration>, Collection<EncryptionConfiguration>,
Collection<GenericJwtClaimsValidator>, JwtAuthenticationFactory)` deprecated previously has been removed.
`JwtTokenValidator(Collection<SignatureConfiguration>, Collection<EncryptionConfiguration>, Collection<GenericJwtClaimsValidator>, JwtAuthenticationFactory, ExecutorService)` is used instead.

- The following methods in `io.micronaut.security.oauth2.client.DefaultOpenIdProviderMetadata` deprecated previously have been removed. Note that the Open ID provider name is now required in all cases:
** Constructor `DefaultOpenIdProviderMetadata()`.
Use `DefaultOpenIdProviderMetadata(String)` instead.
** Static method `Builder builder()`.
Use `DefaultOpenIdProviderMetadata(String)` instead
** Nested constructor `DefaultOpenIdProviderMetadata.Builder()`.
Use `DefaultOpenIdProviderMetadata.Builder(String)` instead

- The Singleton constructor `io.micronaut.security.oauth2.endpoint.authorization.response.DefaultOpenIdAuthorizationResponseHandler(OpenIdTokenResponseValidator, OpenIdAuthenticationMapper, TokenEndpointClient, OauthRouteUrlBuilder<T>, StateValidator, PkcePersistence)` deprecated previously has been removed.
`DefaultOpenIdAuthorizationResponseHandler(OpenIdTokenResponseValidator, OpenIdAuthenticationMapper, TokenEndpointClient, OauthRouteUrlBuilder<T>, StateValidator, PkcePersistence), ExecutorService)` is used instead.
