...
- The configuration property `micronaut.security.csrf.repository.cookie.enabled` is deprecated. Use `micronaut.security.csrf.repositories.cookie.enabled` instead. The deprecated property is still honored when `micronaut.security.csrf.repositories.cookie.enabled` is not set, and a warning is logged.
- The configuration property `micronaut.security.csrf.repository.session.enabled` is accepted as a deprecated alias of `micronaut.security.csrf.repositories.session.enabled`, with the same precedence rules.
