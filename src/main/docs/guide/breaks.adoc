...
- `micronaut.security.token.jwt.signatures.jwks.*.key-type` is now honoured when verifying JWT signatures, and its default value changed from `RSA` to not set (keys of any key type are used). Previously the property was ignored and keys of every key type were tried, so applications which do not set it keep the same behaviour. If you set `key-type` explicitly, only keys of that type are used to verify tokens. `JwksSignatureConfigurationProperties.DEFAULT_KEYTYPE` is deprecated and no longer used.
