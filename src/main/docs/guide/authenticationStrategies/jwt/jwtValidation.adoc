...
WARNING: Unsigned tokens (JWS header `alg=none`) are rejected by default, even when no signature configuration is present, because anyone can forge them. To accept unsigned tokens when no signature configuration is present (for example, in local development), set `micronaut.security.token.jwt.accept-unsigned-tokens` to `true`. When a signature configuration is present, unsigned tokens are always rejected.
