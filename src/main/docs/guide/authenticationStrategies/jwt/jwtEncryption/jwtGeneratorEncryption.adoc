...
<1> `generator.secret` names the Signature configuration `generator` to make it participate in JWT token generation.
<2> `pem.path` specifies the location of PEM file
