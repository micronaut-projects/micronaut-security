...
@Property(name = "spec.name", value = "MultipleAuthenticationProviderAuthenticationResponseFailureSpec")
@Property(name = "micronaut.http.client.follow-redirects", value = StringUtils.FALSE)
@Property(name ="micronaut.security.authentication-provider-strategy", value = "all")
@MicronautTest
class MultipleAuthenticationProviderAuthenticationResponseFailureSpec extends Specification {
    ...
    @Requires(property = "spec.name", value = "MultipleAuthenticationProviderAuthenticationResponseFailureSpec")
    @Singleton
    ...
    @Requires(property = "spec.name", value = "MultipleAuthenticationProviderAuthenticationResponseFailureSpec")
    @Singleton
    static class ReactiveAuthenticationProviderUserPassword<B> implements HttpRequestReactiveAuthenticationProvider<B> {
        ...
        Publisher<AuthenticationResponse> authenticate(
            ...
            AuthenticationResponse rsp =  AuthenticationResponse.success(authRequest.getIdentity())
            Mono.create(emitter -> {
                if (rsp.isAuthenticated()) {
                    emitter.success(rsp)
                } else {
                    emitter.error(new AuthenticationException(rsp))
                }
            })
        }
    }
