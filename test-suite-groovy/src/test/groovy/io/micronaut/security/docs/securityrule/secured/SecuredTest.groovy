...
@MicronautTest
class SecuredTest extends Specification {

    @Inject
    @Client("/")
    HttpClient httpClient

    void "anonymous endpoint can be accessed without authentication"() {
        when:
    ...
    void "authenticated endpoint requires an authenticated user"() {
        given:
        BlockingHttpClient client = httpClient.toBlocking()

        when:
        client.exchange(HttpRequest.GET("/example/authenticated"))
        ...
        when:
        client.exchange(HttpRequest.GET("/example/authenticated").basicAuth("user", "password"))

        then:
        noExceptionThrown()
    }

    void "admin endpoint requires one of the roles"() {
        given:
        BlockingHttpClient client = httpClient.toBlocking()
