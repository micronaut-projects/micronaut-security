...
class AuthenticationWebSocketStateBinderSpec extends Specification {
    ...
    void "authentication can be injected into websocket @OnOpen"() {
        when:
        ...
        MutableHttpRequest<?> request = HttpRequest.GET("http://localhost:${server.port}/auth-echo").bearerAuth(token)
    ...
    private static Optional<String> generateJwt(TokenGenerator tokenGenerator) {
        ...
        tokenGenerator.generateToken(Authentication.build("john"), expiration)
    }
...
//tag::onOpen[]
        ...
        void onOpen(WebSocketSession session, Authentication authentication) {
            broadcaster.broadcastSync("joined! ${authentication.name}".toString(), isValid(session))
        }
//end::onOpen[]

        @OnMessage
        void onMessage(String message, WebSocketSession session, Authentication authentication) {
            broadcaster.broadcastSync("I received a message from ${authentication.name}".toString(), isValid(session))
        }

        private static Predicate<WebSocketSession> isValid(WebSocketSession session) {
            return { WebSocketSession s -> s == session } as Predicate<WebSocketSession>
        }
    }
