...
@Requires(property = PROPERTY_ALLOW_SAME_SITE, value = StringUtils.TRUE, defaultValue = StringUtils.FALSE)
@Singleton
@Internal
final class SameSiteFetchMetadataRule implements HttpRequestFetchMetadataRule {
    @Override
    public int getOrder() {
        return FetchMetadataRuleOrder.SAME_SITE;
    }

    @Override
    public FetchMetadataRuleResult check(HttpRequest<?> request, @Nullable SecFetch secFetch) {
        if (secFetch != null && secFetch.site() == Site.SAME_SITE) {
