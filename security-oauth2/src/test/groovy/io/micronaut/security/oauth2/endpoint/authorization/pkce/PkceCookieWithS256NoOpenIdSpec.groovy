...
@Narrative("PKCE with cookie persistence for a manual OAuth 2.0 Client configuration which does not use OpenID Autoconfiguration but it specifies the code-challenge-method manually")
class PkceCookieWithS256NoOpenIdSpec extends Specification {
    void "PKCE cookie persistence Manual OAuth 2.0 Client code-challenge-method manually"() {
        ...
        EmbeddedServer oauthServer = ApplicationContext.run(EmbeddedServer, [
            "spec.name": "AuthServerPkceCookieWithS256NoOpenIdSpec",
        ...
        EmbeddedServer server = ApplicationContext.run(EmbeddedServer, [
            "spec.name": "PkceCookieWithS256NoOpenIdSpec",
            "micronaut.security.authentication": "cookie",
            "micronaut.security.oauth2.pkce.persistence": "cookie",
            'micronaut.http.client.followRedirects': false,
            "micronaut.security.oauth2.pkce.enabled": StringUtils.TRUE,
            "micronaut.security.oauth2.clients.auth.authorization.code-challenge-method": "S256",
            "micronaut.security.oauth2.clients.auth.authorization.url": "http://localhost:${oauthServer.port}/oauth2/default/v1/authorize".toString(),
            "micronaut.security.oauth2.clients.auth.token.url": "http://localhost:${oauthServer.port}/oauth2/default/v1/token".toString(),
        ...
        server.applicationContext.containsBean(OauthClientConfiguration)
        server.applicationContext.getBean(OauthClientConfiguration).getAuthorization().isPresent()
        server.applicationContext.getBean(OauthClientConfiguration).getAuthorization().get().codeChallengeMethod.isPresent()
        "S256" == server.applicationContext.getBean(OauthClientConfiguration).getAuthorization().get().codeChallengeMethod.get()
        ...
        Cookie cookiePkce = response.cookies.get("OAUTH2_PKCE")
    ...
    @Requires(property = "spec.name", value="PkceCookieWithS256NoOpenIdSpec")
    @Singleton
    @Named("auth")
    static class MockOauthAuthenticationMapper implements OauthAuthenticationMapper {
        ...
        Publisher<AuthenticationResponse> createAuthenticationResponse(TokenResponse tokenResponse, @Nullable State state) {
            Flux.create({ emitter ->
                emitter.next(AuthenticationResponse.success("john"))
                emitter.complete()
    ...
    @Requires(property = "spec.name", value="AuthServerPkceCookieWithS256NoOpenIdSpec")
    @Secured(SecurityRule.IS_ANONYMOUS)
    @Controller
    static class AuthServerController {
        ...
        HttpResponse<?> token(HttpRequest<?> request, @Body AuthorizationCodeGrant codeGrant) {
            ...
            if (S256PkceGenerator.hash(codeVerifier) != codeChallenge) {
                return HttpResponse.unprocessableEntity()
            }
