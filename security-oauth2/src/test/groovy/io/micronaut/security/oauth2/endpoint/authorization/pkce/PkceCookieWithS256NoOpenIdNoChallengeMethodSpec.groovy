...
@Narrative("PKCE with cookie persistence for a manual OAuth 2.0 Client configuration which does not use OpenID Autoconfiguration and it does not specifies the code-challenge-method manually. Hence no PKCE")
class PkceCookieWithS256NoOpenIdNoChallengeMethodSpec extends Specification {
    void "PKCE Manual OAuth No code-challenge specified"() {
        ...
        EmbeddedServer oauthServer = ApplicationContext.run(EmbeddedServer, [
            "spec.name": "AuthServerPkceCookieWithS256NoOpenIdNoChallengeMethodSpec",
        ...
        EmbeddedServer server = ApplicationContext.run(EmbeddedServer, [
            "spec.name": "PkceCookieWithS256NoOpenIdNoChallengeMethodSpec",
    ...
    @Requires(property = "spec.name", value="PkceCookieWithS256NoOpenIdNoChallengeMethodSpec")
    @Singleton
    @Named("auth")
    ...
    @Requires(property = "spec.name", value="AuthServerPkceCookieWithS256NoOpenIdNoChallengeMethodSpec")
    @Secured(SecurityRule.IS_ANONYMOUS)
    @Controller
    static class AuthServerController {
        ...
        HttpResponse<?> token(HttpRequest<?> request, @Body AuthorizationCodeGrant codeGrant) {
            ...
            if (StringUtils.isNotEmpty(codeVerifier)) {
