...
class OpenIdIssuerTokenNotManuallyConfiguredConditionSpec extends Specification {
    @Shared
    ...
    void "#description"(boolean expected, String name) {
        expect:
        expected == !OpenIdIssuerTokenNotManuallyConfiguredCondition.tokenEndpointIsNotManuallyConfiguredAndItContainsOpenIdIssuer(beanContext, name).isPresent()

        where:
        expected || name
        true     || 'authserveropenid'
        false    || 'authservermanual'
        false    || 'authservermanualtakesprecedenceoveropenid'
