...
final class ScimDiscoveryController {
    ...
    @SingleResult
    Publisher<MutableHttpResponse<?>> serviceProviderConfiguration(HttpRequest<?> request) {
        ScimRequestContext context = requestParser.context(request, ScimAttributeSelection.ALL);
        return Mono.from(service.getServiceProviderConfiguration(context))
            ...
            .map(result -> response(result.resource(), result.location().toString(), result.version()));
    }

    @Get("/Schemas{?filter}")
    @SingleResult
    Publisher<MutableHttpResponse<?>> schemas(@Nullable @QueryValue String filter, HttpRequest<?> request) {
    ...
    @Get("/Schemas/{id}")
    @SingleResult
    Publisher<MutableHttpResponse<?>> schema(String id, HttpRequest<?> request) {
    ...
    @SingleResult
    Publisher<MutableHttpResponse<?>> resourceTypes(@Nullable @QueryValue String filter, HttpRequest<?> request) {
        rejectDiscoveryFilter(filter);
    ...
    @SingleResult
    Publisher<MutableHttpResponse<?>> resourceType(String id, HttpRequest<?> request) {
        return item(service.getResourceTypes(requestParser.context(request, ScimAttributeSelection.ALL)), id, request);
    }

    private static <T extends ScimResource> Publisher<MutableHttpResponse<?>> list(Publisher<T> resources) {
        return Flux.from(resources)
            .collectList()
            .map(values -> HttpResponse.ok(new ScimListResponse<>(List.of(ScimMessageSchemas.LIST_RESPONSE),
                    values.size(), values, 1, values.size()))
                .contentType(ScimMediaType.APPLICATION_SCIM_JSON_TYPE));
    }

    private static <T extends ScimResource> Publisher<MutableHttpResponse<?>> item(
        Publisher<T> resources,
        String id,
        HttpRequest<?> request
    ) {
        return Flux.from(resources)
